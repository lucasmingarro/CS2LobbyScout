...
describe('computeScore', () => {
  ...
  it('scores the spec example "aim.exe" as high/very high with explainable signals', () => {
    ...
    expect(types).toEqual(expect.arrayContaining(['kd_high', 'adr_high', 'hs_high', 'low_match_count', 'young_account', 'win_rate_high']))
  ...
  it('never exceeds 100 and caps each component', () => {
    ...
    expect(r.components.kd).toBe(THRESHOLDS.kd.max)
    expect(r.components.adr).toBe(THRESHOLDS.adr.max)
    expect(r.components.hs).toBe(THRESHOLDS.hs.max)
    expect(r.components.performanceJump).toBe(10)
    expect(r.components.winRate).toBe(THRESHOLDS.winRate.max)
  ...
  it('ramps linearly between thresholds', () => {
    const mid = (THRESHOLDS.kd.from + THRESHOLDS.kd.to) / 2
    const r = computeScore({ faceit: { matches: 500, kd: mid } }, NOW)
    expect(r.components.kd).toBe(Math.round(THRESHOLDS.kd.max / 2))
    expect(computeScore({ faceit: { matches: 500, kd: THRESHOLDS.kd.from } }, NOW).components.kd).toBe(0)
  })
  ...
  it('detects a recent performance jump only with enough history', () => {
    ...
    expect(jump.signals.find((s) => s.type === 'performance_jump')?.explanation).toMatch(/Last 20 matches/)
